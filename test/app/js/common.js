...
var paginationElems = document.querySelectorAll('.pagination-elem');
var prevButton = document.querySelector('.prev-button');
var nextButton = document.querySelector('.next-button');
var counter = 0;

prevButton.addEventListener('click', prevButtonSwitch);
nextButton.addEventListener('click', nextButtonSwitch);
for (var i = 0; i < tabsLinks.length; i++) {
	tabsLinks[i].addEventListener('click', tabsLinksSwitch);
}
for (var i = 0; i < paginationElems.length; i++) {
	paginationElems[i].addEventListener('click', paginationElemsSwitch);
}
...
function startSlider() {
	window.timerId = window.setInterval(function() {
		...
		for (var i = 0; i < slides.length; i++) {
			...
			paginationElems[counter].classList.add('active');
			tabsLinks[i].classList.remove('active');
			tabsLinks[counter].classList.add('active');
		}
	}, 1000);
}
startSlider();
...
function tabsLinksSwitch(argument) {
	stopSlider();
	for (var i = 0; i < tabsLinks.length; i++) {
		tabsLinks[i].classList.remove('active');
...
function paginationElemsSwitch() {
	stopSlider();
	for (var i = 0; i < paginationElems.length; i++) {
		paginationElems[i].classList.remove('active');
		this.classList.add('active');
		if (paginationElems[i].classList.contains('active')) {
			counter = i;
		}
	}
	for (var i = 0; i < slides.length; i++) {
