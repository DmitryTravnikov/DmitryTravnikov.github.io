...
$(document).ready(function() {
	...
	var topPanelToggleButton = document.querySelector('.top-panel__toggle-button');
	var header = document.querySelector('.header');
	topPanelToggleButton.onclick = function() {
		this.classList.toggle('active');
		if (this.classList.contains('active')) {
			topPanel.style.top = '0rem';
	...
	function validationCheck() {
		for (var i = 0; i < validationArr.length; i++) {
			if (!validationArr[i].value) {
				validationArr[i].style.borderColor = 'orange';
				modalErrorShow();
			} else {
		...
		if (footerNameInput.value && footerEmailInput.value && footerEmailInput.value && footerMessageInput.value) {
			modalSuccessShow();
		}
	}
	...
	function modalsHide() {
		...
		for (var i = 0; i < modalWindows.length; i++) {
			modalWindows[i].style.opacity = '';
			modalWindows[i].style.zIndex = '';
		}
	}
