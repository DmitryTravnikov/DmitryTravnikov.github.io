...
$(document).ready(function() {
	...
	//hamburger n menu appearance
	var topPanelHamburger = document.querySelector('.top-panel__hamburger');
	var topPanelMenuOverlay = document.querySelector('.top-panel__menu-overlay');
	topPanelHamburger.onclick = function() {
		for (var i = 0; i < this.children.length; i++) {
			this.children[i].classList.toggle('active');
		}
		topPanelMenuOverlay.classList.toggle('active');
	}

	//ajax start
	$('.portfolio__menu-link').click(function() {

		$('.portfolio__menu-link').removeClass('active');
	...
	var portfolioVideoPlayButton = document.querySelector('.portfolio-vid__video-play-button');
	...
	//smooth scroll
	$('.header__arrow-icon, .footer__logo, .top-panel__logo, .top-panel__menu-link').mPageScroll2id({
	  offset: 0,
	  scrollSpeed: 200
	});
	$('.top-panel__menu-link').click(function() {
		topPanelMenuOverlay.classList.remove('active');
		for (var i = 0; i < topPanelHamburger.children.length; i++) {
			topPanelHamburger.children[i].classList.remove('active');
		}
	});
