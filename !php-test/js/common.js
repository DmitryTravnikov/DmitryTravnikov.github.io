...
$(document).ready(function() {
	...
	$('#form').submit(function() {
		$.ajax({
			type: 'POST',
			url: 'mail.php',//ссылка на mail.php
			data: $(this).serialize()//собираем все поля формы в массив
		}).done(function() {//при успешной отправке
			alert('Спасибо за заявку! Скоро мы с вами свяжемся.');//выдается сообщение
		});
		return false;
	});
